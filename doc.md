...
`oi_func void pack(void * net, void * source, size_t len)`  
Maps the given source array to the net array. Simply a wrapper for memcpy.

`oi_func void unpack(void * net, void * dest, size_t len)`  
Maps the net array to the given dest array. Simply a wrapper for memcpy.

`oi_func void pack8(void * net, uint8 source)`  
Maps the given source to the net buffer.

`oi_func uint8 unpack8(void * net)`  
Maps the net buffer to an uint8.

`oi_func void pack16(void * net, uint16 source)`  
Maps the given source to the net buffer.

`oi_func uint16 unpack16(void * net)`  
Maps the net buffer to an uint16.

`oi_func void pack32(void * net, uint32 source)`  
Maps the given source to the net buffer.

`oi_func uint32 unpack32(void * net)`  
Maps the net buffer to an uint32.

`oi_func void pack64(void * net, uint64 source)`  
Maps the given source to the net buffer.

`oi_func uint64 unpack64(void * net)`  
Maps the net buffer to an uint64.

`oi_func void packf32(void * net, float32 source)`  
Maps the given source to an IEEE repressentation on the net buffer.

`oi_func float32 unpack32(void * net)`  
Maps the net buffer to a float32.

`oi_func void packf64(void * net, float64 source)`  
Maps the given source to an IEEE repressentation on the net buffer.

`oi_func float64 unpack64(void * net)`  
...
`oi_call address_from_ipv4(address_t * addr, void * ip_address, uint16 port)`  
Creates an address in addr from the raw IPv4 address and port. The address must have a size of 4.

`oi_call address_from_ipv6(address_t * addr, void * ip_address, uint16 port)`  
Creates an address in addr from the raw IPv6 address and port. The address must have a size of 16.

`oi_call address_from_name(address_t * addr, const char * name, uint16 port, int lookup`  
...
`oi_call socket_create_on(socket_t * s, int proto, address_t * addr)`  
...
`oi_call udp_send(socket_t * s, void * buffer, size_t * len, address_t * addr)`  
...
`oi_call udp_rec_any(socket_t ** select, void * buffer, size_t * len, address_t * addr, unsigned int ms, int number, socket_t * s1, ...)`  
...
`oi_call tcp_connect_any(socket_t ** select_s, address_t ** select_a, unsigned int ms, int number, socket_t * s1, address_t * a1, ...)`  
...
`oi_call tcp_accept_any(socket_t ** select, socket_t * connectsock, address_t * addr, unsigned int ms, int number, socket_t * s1, ...)`  
...
`oi_func int socket_get_nodelay(socket_t * s)`  
Gets wether or not the current socket has the nodelay option enabled.

`oi_call socket_set_keepalive(socket_t * s, size_t len)`  
...
`oi_func int socket_get_keepalive(socket_t * s)`  
